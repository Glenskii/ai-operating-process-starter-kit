...
Read `.ai-operating-process/AI-SESSION-BRIEF.md` and `.ai-operating-process/project-context.md` before non-trivial work.
...
def main() -> int:
    ...
    generated = {
        ...
        "README.md": """# AI Operating Process Bridge\n\nThis folder binds the repository to a personal AI Operating Process without committing machine-specific paths, secrets, or session state.\n\n- `AI-SESSION-BRIEF.md` is the vendor-neutral starting point for any AI tool.\n- `project-context.md` is the project-specific context and delivery runbook.\n- `config.local.json` is ignored and stores the local process root.\n- `handoff.md` is ignored and stores continuity before compaction or an agent handoff.\n- `CHATGPT-PROJECT-PACKET.md` is an optional adapter for a ChatGPT Project.\n""",
        "AI-SESSION-BRIEF.md": """# AI Session Brief\n\nUse this file with any AI assistant that works on this project.\n\n1. Read `project-context.md` before non-trivial work.\n2. Use the local process path in `config.local.json` when it is available. Do not invent a path if it is missing.\n3. Select a work mode, load only the relevant routine, and respect approval gates.\n4. Run live preflight before production-impacting actions.\n5. Save decisions, validation, risk, and the next step to `handoff.md` before compaction or a cross-agent handoff.\n\nDo not claim access to files, tools, deployments, or live systems that are not available in the current session.\n""",
